...
#   labs(
#     x = "Density",
#     y = "Sample size in meta-analysis",
#     title = "Violin and box plot"
#   ) +
...
#   )
# 
# #Violin plot of number of studies included in the meta-analysis
# plot_df <- full %>%
#   distinct(Study, .keep_all = TRUE) %>%
#   mutate(Number.of.studies = as.numeric(Number.of.studies)) %>%
#   filter(!is.na(Number.of.studies))
# 
# p_d <- ggplot(plot_df, aes(x = "Density", y = Number.of.studies)) +
#   geom_violin(
#     trim = FALSE,
#     fill = "white",
#     color = "grey40",
...
#   ) +
#   scale_y_continuous(
#     limits = c(0, max(plot_df$Number.of.studies, na.rm = TRUE) * 1.05),
#     expand = expansion(mult = c(0, 0.03))
#   ) +
#   labs(
#     x = "Density",
#     y = "Number of studies in meta-analysis",
#     title = "Violin and box plot"
#   ) +
#   theme_gray(base_size = 14) +
#   theme(
...
# --- OPTION A: sign-flip reverse effects ---
df <- df %>%
  mutate(
    d_unified = d,
...
# =============================================================================
# SECTION 8: SUBGROUP ANALYSIS — DIRECTION
# =============================================================================

cat("\n======================================================\n")
cat("MODEL 2: Subgroup — forward vs. reverse direction\n")
cat("======================================================\n")

res_direction <- rma.mv(
  yi = d_unified,
  V  = vi,
  mods = ~ direction_factor,
...
# =============================================================================
# SECTION 9: FULL META-REGRESSION
#
# Moderators: direction, follow-up duration, mean age, log sample size,
#             proportion female, region
# =============================================================================
...
# =============================================================================
# SECTION 10: ROBUST STANDARD ERRORS (cluster-robust, Tipton 2015)
#
# Accounts for dependency when multiple effects come from the same study.
# =============================================================================

cat("\n======================================================\n")
cat("MODEL 3 (robust SEs via clubSandwich)\n")
cat("======================================================\n")

if (requireNamespace("clubSandwich", quietly = TRUE)) {
  library(clubSandwich)
  robust_full <- coef_test(
    res_full,
    vcov = "CR2",
    cluster = df_complete$cluster_id
  )
  print(robust_full)
} else {
  cat("Package 'clubSandwich' not installed. Run:\n")
  cat("  install.packages('clubSandwich')\n")
}
...
if (nrow(df_forward) > 0) {
  k <- nrow(df_forward)
  row_spacing <- 1.5
  
  row_pos <- seq(from = k * row_spacing, by = -row_spacing, length.out = k)
  
# png("forest_plot_forward.png",
#       width  = 750,
#       height = max(600, k * 35),
#       res    = 120)
  
  # SVG — best for web or editing in Illustrator/Inkscape
  svg("forest_plot_forward.svg", width = 7.5, height = max(6, k * 0.35))
  ...
  dev.off()
  cat("Saved: forest_plot_forward.svg\n")
...
png("funnel_plot.png", width = 900, height = 700, res = 120)
funnel(
  res_re_simple,
  main = "Funnel Plot (simple RE model)",
  xlab = "Cohen's d",
  col  = "steelblue"
)
dev.off()
cat("Saved: funnel_plot.png\n")


# ---------- 13D: Bubble plot — effect size vs. follow-up duration ----------

df_plot <- df %>% filter(!is.na(followup_months))
...
p_bubble <- ggplot(df_plot, aes(
  ...
  labs(
    ...
    y      = "Cohen's d (unified direction)",
    colour = NULL
  ) +
  theme_bw(base_size = 12) +
  theme(legend.position = "bottom")

ggsave("bubble_followup.png", p_bubble, width = 10, height = 6, dpi = 150)
cat("Saved: bubble_followup.png\n")


# ---------- 13E: Bubble plot — effect size vs. mean age ----------

df_plot2 <- df %>% filter(!is.na(age_mean))
...
ggsave("bubble_age.png", p_age, width = 10, height = 6, dpi = 150)
cat("Saved: bubble_age.png\n")
